...
static void tick(VMAC_IP_UDP_rx *top, VerilatedFstC *tfp) {
    top->CLK = 1;
...
void reset_module(VMAC_IP_UDP_rx *top, VerilatedFstC *tfp) {
    top->xgmii_rxd = 0;
    top->xgmii_rxc = 0;
...
static void drive_idle(VMAC_IP_UDP_rx *dut, VerilatedFstC *tfp, int cycles = 1)
{
...
static void drive_col(VMAC_IP_UDP_rx *dut, VerilatedFstC *tfp, const Column &c)
{
    dut->xgmii_rxc = c.ctl;
    dut->xgmii_rxd = c.dat;
    tick(dut, tfp);
...
// Generic test runner
// ----------------------------------------------------------------------------
static bool run_frame(VMAC_IP_UDP_rx          *dut,
                      VerilatedFstC    *tfp,
...
// ----------------------------------------------------------------------------
static bool test_good_L0 (VMAC_IP_UDP_rx *dut, VerilatedFstC *tfp)
{ return run_frame(dut, tfp, GOOD_L0,  "GOOD_L0",  true,  false); }

static bool test_good_L4 (VMAC_IP_UDP_rx *dut, VerilatedFstC *tfp)
{ return run_frame(dut, tfp, GOOD_L4,  "GOOD_L4",  true,  false); }

static bool test_frame (VMAC_IP_UDP_rx * dut, VerilatedFstC *tfp, std::vector<Column> frame)
{ return run_frame(dut, tfp, frame, "frame generated", true, false);}

// static bool test_bad_crc (VMAC_IP_UDP_rx *dut, VerilatedFstC *tfp)
...
int main(int argc, char **argv) {
    ...
    VMAC_IP_UDP_rx *top = new VMAC_IP_UDP_rx;

    Verilated::traceEverOn(true);
    VerilatedFstC *tfp = new VerilatedFstC;
    top->trace(tfp, 99);
    tfp->open("MAC_IP_UDP_rx.fst");
    top -> nRST = 0;
