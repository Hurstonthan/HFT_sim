...
std::uint32_t rd_FIFO_ptr;
std::uint32_t rd_FIFO_len;
...
void drive_input_payload_FIFO(std::uint8_t CLK_in, std::uint8_t nRST_in, std::uint8_t nw_segment_in, std::uint8_t handshake_done_in, std::uint32_t seq_rcv_start_in, std::uint8_t TCP_flush_in, std::uint8_t axis_t_last_in, std::uint8_t wr_FIFO_en_in, std::uint64_t axis_data_rx_in, std::uint8_t wr_FIFO_offset_in, std::uint8_t rd_FIFO_valid_in, std::uint32_t rd_FIFO_ptr_in, std::uint32_t rd_FIFO_len_in, std::uint8_t axis_r_en_in) {
    CLK = CLK_in;
    nRST = nRST_in;
