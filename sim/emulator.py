	...
	def _recv_thread(self):
		...
		while True:
			try:
				...
				# print(f"Emulator got data={data}")
...
class SendingQueue:
	...
	def add(self, packets):
		...
		for packet in packets:
			# print(f"before self._queue={self._queue}")
			drop_count = max(0, len(self._queue) + 1 - Config.MAX_PACKETS_QUEUED)
			self._queue = self._queue[:Config.MAX_PACKETS_QUEUED]
			# print(f"drop_count={drop_count}")
			# print(f"len(self._queue)={len(self._queue)}")
			# print(f"Config.MAX_PACKETS_QUEUED={Config.MAX_PACKETS_QUEUED}")
			# print(f"after self._queue={self._queue}")
