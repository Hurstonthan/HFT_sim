...
VMAC_rx::VMAC_rx(VerilatedContext* _vcontextp__, const char* _vcname__)
    ...
    , __PVT__MAC_rx{vlSymsp->TOP.__PVT__MAC_rx}
...
VL_ATTR_COLD void VMAC_rx::traceBaseModel(VerilatedTraceBaseC* tfp, int levels, int options) {
    (void)levels; (void)options;
    VerilatedFstC* const stfp = dynamic_cast<VerilatedFstC*>(tfp);
    if (VL_UNLIKELY(!stfp)) {
        vl_fatal(__FILE__, __LINE__, __FILE__,"'VMAC_rx::trace()' called on non-VerilatedFstC object;"
            " use --trace-fst with VerilatedFst object, and --trace-vcd with VerilatedVcd object");
    }
    stfp->spTrace()->addModel(this);
    stfp->spTrace()->addInitCb(&trace_init, &(vlSymsp->TOP));
    VMAC_rx___024root__trace_register(&(vlSymsp->TOP), stfp->spTrace());
}
